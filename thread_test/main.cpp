...
void workerfunc(int id, std::mt19937& generator)
{
	...
	{
		std::unique_lock<std::mutex> locker_q(g_lockqueue);
...
void loggerfunc()
{
	...
	while(!g_done)
	{
		std::unique_lock<std::mutex> locker_c(g_lockcondition);
		while(!g_notified) // used to avoid spurious wakeups 
		{
			g_queuecheck.wait(locker_c);
		}

		{
			...
			while(!g_codes.empty())
			{
				std::unique_lock<std::mutex> locker_p(g_lockprint);
